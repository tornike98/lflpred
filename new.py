...
def is_forecast_open() -> bool:
    ...
    weekday = moscow_time.weekday()  # Пн=0, Вт=1, Ср=2, Чт=3, Пт=4, Сб=5, Вс=6
    current_time = moscow_time.time()

    # Если сегодня вторник, прием открыт только с 18:00
    if weekday == 1:
        return current_time >= time(18, 0)
    # Если сегодня пятница, прием открыт до 23:00
    elif weekday == 4:
        return current_time < time(21, 0)
    # Если сегодня среда или четверг, прогнозы принимаются
    elif weekday in (2, 3):
        return True
    # В остальные дни (понедельник, суббота, воскресенье) прием прогнозов закрыт
    else:
        return False
